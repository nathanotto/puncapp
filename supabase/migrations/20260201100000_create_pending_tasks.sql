...
create index if not exists idx_pending_tasks_user_pending
...
create index if not exists idx_pending_tasks_entity
...
create policy if not exists "Users can view their own pending tasks"
  on pending_tasks
  for select
  using (auth.uid() = assigned_to);

-- Policy: Users can update their own pending tasks (mark as completed/dismissed)
create policy if not exists "Users can update their own pending tasks"
  on pending_tasks
  for update
  using (auth.uid() = assigned_to);
