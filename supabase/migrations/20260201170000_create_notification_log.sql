...
CREATE POLICY IF NOT EXISTS "Authenticated users can view notification log" ON notification_log
  FOR SELECT USING (auth.role() = 'authenticated');

-- System inserts (service role), but for testing allow authenticated
CREATE POLICY IF NOT EXISTS "Authenticated users can insert notifications" ON notification_log
