...
export async function POST(req: NextRequest) {
  try {
    ...
    // Update meeting to full_checkins if not already there
    if (meeting.current_section !== 'full_checkins') {
      await supabase
        .from('meetings')
    ...
    if (!existingSectionLog) {
      ...
      const sectionStartTime = new Date(now.getTime() - (attendees.length * 15 * 60 * 1000) - (5 * 60 * 1000))
    ...
    // Create full checkin logs for each attendee
    const logs = attendees.map((attendee, index) => {
      // Realistic durations: 12-18 minutes (720-1080 seconds)
      const baseDuration = 720 + Math.floor(Math.random() * 360)
      const overtime = Math.random() > 0.6 ? Math.floor(Math.random() * 60) : 0
      const duration = baseDuration - overtime

      // Randomly assign stretch goal actions
      const actions = ['kept', 'completed', 'new', 'none']
      const stretchGoalAction = actions[Math.floor(Math.random() * actions.length)]

      // 20% chance of requesting support
      const requestedSupport = Math.random() > 0.8

      // Stagger start times by ~15 minutes each
      const startTime = new Date(now.getTime() - (attendees.length - index) * 15 * 60 * 1000)
    ...
    console.error('Error accelerating checkins:', error)
    return NextResponse.json({
      success: false,
